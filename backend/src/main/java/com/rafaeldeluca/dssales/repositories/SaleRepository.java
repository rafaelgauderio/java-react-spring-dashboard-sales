package com.rafaeldeluca.dssales.repositories;
...
public interface SaleRepository extends JpaRepository<Sale, Long> {
	...
	@Query("SELECT new com.rafaeldeluca.dssales.dto.SalesByStoreDTO(obj.store, SUM(obj.total)) "
			+ "FROM Sale AS obj "
	...
	@Query("SELECT new com.rafaeldeluca.dssales.dto.SalesByPaymentMethodDTO(obj.paymentMethod, SUM(obj.total)) "
			+ "FROM Sale AS obj "
			+ "WHERE (CAST(:min AS date) IS NULL OR obj.date >= :min) "
			+ "AND (CAST(:max AS date) IS NULL OR obj.date <= :max) "
			+ "AND (:genderEnum IS NULL OR obj.gender = :genderEnum) "
	...
	@Query("SELECT new com.rafaeldeluca.dssales.dto.SalesByDateDTO(obj.date, SUM(obj.total)) "
			+ "FROM Sale AS obj "
			+ "WHERE (CAST(:min AS date) IS NULL OR obj.date >= :min) "
			+ "AND (CAST(:max AS date) IS NULL OR obj.date <= :max) "
			+ "AND (:genderEnum IS NULL OR obj.gender = :genderEnum) "
	...
	@Query("SELECT new com.rafaeldeluca.dssales.dto.SalesSummaryDTO(SUM(obj.total), MAX(obj.total), MIN(obj.total), AVG(obj.total), COUNT(obj.id)) "
			+ "FROM Sale AS obj "
			+ "WHERE (CAST(:min AS date) IS NULL OR obj.date >= :min) "
			+ "AND (CAST(:max AS date) IS NULL OR obj.date <= :max) "
			+ "AND (:genderEnum IS NULL OR obj.gender = :genderEnum) ")
